...
class MiniCart_Settings
{
    ...
    public function pro_settting_sec_func()
    {
        ...
        require_once MCFWC_LIGHT_PATH . '/admin/pro-features.php';
    ...
    $yoo_type_pages = get_posts(array(
        ...
        $options = get_option('whmc_option');
            ...
            printf('<div><input type="checkbox" id="%s"  value="%s" name="whmc_option[%s]" %s><label for ="%s" >' . $yoo_type_page->post_title . '</label></div>', $yoo_type_page->ID, $yoo_type_page->ID, $yoo_type_page->ID, $checked,$yoo_type_page->ID);
    ...
    $excluded_posttypes = array('attachment','revision','nav_menu_item','custom_css','customize_changeset','oembed_cache','user_request','wp_block','scheduled-action','product_variation','shop_order','shop_order_refund','shop_coupon','elementor_library','e-landing-page','page','wp_navigation','wp_template_part','wp_global_styles','wp_template','shop_order_placehold');
        ...
        foreach ($post_types as $post_type)
        {
            ...
            printf('<div><input type="checkbox" id="%s"  value="%s" name="whmc_option[%s]" %s><label for ="%s" >' . $post_type_title->labels->name . '</label></div>', $post_type, $post_type, $post_type, $checkedposttype, $post_type);
  ...
  <select id="wmhc_footer_bag_ficon"  class="default" name="whmc_option[wmhc_footer_bag_ficon]">
    ...
    public function fcp_fotter_cart_size()
    {
        ...
        printf('<input type="range" name="whmc_option[fcp_fotter_cart_size]" value="%s" min="20" max="150" id="fcp_fotter_cart_size" oninput="fcarsiz.value = this.value"><output id="fcarsiz">%s</output> <span class="dashicons dashicons-image-rotate" id="fcp_fartsizeundo"></span>', $value,$value);
    ...
    public function fcp_option_range()
    {
        ...
        printf('<input type="range" name="whmc_option[fcp_option_range]" value="%s" min="0" max="50"  id="fcp_option_range" oninput="hrang.value = this.value"><output id="hrang">%s</output> <span class="dashicons dashicons-image-rotate" id="fcprangeundo"></span>',$value,$value);
    ...
    public function fcp_verticalmove()
    {
        ...
        printf('<input type="range" name="whmc_option[fcp_option_range_bottom]" value="%s" min="0" max="50" id="fcp_option_range_bottom" oninput="vrang.value = this.value"><output id="vrang">%s</output> <span class="dashicons dashicons-image-rotate" id="fcprangbottomeundo"></span>', $value,$value);
    ...
    public function fcp_cart_color(){
        ...
        printf('<input type="text" name="whmc_option[fcp_cart_color]" value="%s" id="fcp_cart_color">', $options_rang_value);
    }
    public function fcp_cart_bgs(){
        ...
        printf('<input type="text" name="whmc_option[fcp_cart_bgs]" value="%s" id="fcp_cart_bgs">', $fcp_cart_bgs);
    }
    ...
    public function fcp_cart_bubble_bg_color(){
        ...
        printf('<input type="text" name="whmc_option[fcp_cart_bubble_bg_color]" value="%s" id="fcp_cart_bubble_bg_color">', $fcp_cart_bubble_bg_color);
    }    
    ...
    public function fcp_cart_bubble_color(){
       ...
       $fcp_cart_bubble_color = isset($options['fcp_cart_bubble_color']) ? $options['fcp_cart_bubble_color'] : '#282828';
        ...
        printf('<input type="text" name="whmc_option[fcp_cart_bubble_color]" value="%s" id="fcp_cart_bubble_color">', $fcp_cart_bubble_color);
    }
    ...
    public function wmhc_hide_footer_cart()
    {
        ...
        printf('<input type="checkbox" name="whmc_option[wmhc_hide_footer_cart]" class="whmc_apple-switch"  value="wmhc_hide_footer_cart" %s id="wmhc_hide_footer_cart"><p class="whmc_description">'.__('Click to remove the footer cart icon from the entire website','mini-cart-for-woocommerce').'</p>', (isset($options['wmhc_hide_footer_cart']) && $options['wmhc_hide_footer_cart'] === 'wmhc_hide_footer_cart') ? 'checked' : '');
